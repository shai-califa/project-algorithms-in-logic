...
inline void Solver::reset_iterators_Q(double where) {
	m_Q_Score2Vars_it = (where == 0) ? m_Q_Score2Vars.begin() : m_Q_Score2Vars.lower_bound(where);
	Assert(m_Q_Score2Vars_it != m_Q_Score2Vars.end());
...
void Solver::reset_after_assigment(Var v)
{
	participated[v] = 0;
	reasoned[v] = 0;
...
void Solver::bumpQScore(int var_idx)
{
	...
	if (score > 0) {
		Assert(m_Q_Score2Vars.find(score) != m_Q_Score2Vars.end());
		m_Q_Score2Vars[score].erase(var_idx);
		if (m_Q_Score2Vars[score].size() == 0) m_Q_Score2Vars.erase(score);
	}
	double LR;
	double RSR;
	...
	new_score = (1 - alpha) * factors[var_idx] * score + alpha * (LR + RSR);
	m_Q[var_idx] = new_score;
	factors[var_idx] = 1;
	if (m_Q_Score2Vars.find(new_score) != m_Q_Score2Vars.end())
		m_Q_Score2Vars[new_score].insert(var_idx);
	else
		m_Q_Score2Vars[new_score] = unordered_set<int>({ var_idx });
}
...
SolverState Solver::decide(){
	...
	case  VAR_DEC_HEURISTIC::ERWA: {
		if (m_should_reset_iterators) reset_iterators_Q(m_curr_Q);
		Var v = 0;
		int cnt = 0;
		if (m_Q_Score2Vars_it == m_Q_Score2Vars.end()) break;
		while (true) { // scores from high to low
			while (m_VarsSameQ_it != m_Q_Score2Vars_it->second.end()) {
				v = *m_VarsSameQ_it;
				++m_VarsSameQ_it;
				++cnt;
				if (state[v] == VarState::V_UNASSIGNED) { // found a var to assign
	...
	case  VAR_DEC_HEURISTIC::ERWA_RSR: {
		if (m_should_reset_iterators) reset_iterators_Q(m_curr_Q);
		Var v = 0;
		int cnt = 0;
		if (m_Q_Score2Vars_it == m_Q_Score2Vars.end()) break;
...
int Solver::analyze(const Clause conflicting) {
	...
	Clause	current_clause = conflicting, // falsified clause 
			new_clause; // learnt clause
	int resolve_num = 0, // # variables assignd at current decition level yet to be treated
		bktrk = 0, // back track level 
		watch_lit = 0, // points to what literal in the learnt clause should be watched, other than the asserting one
		antecedents_idx = 0;
	Lit u;
	Var v;
	vector<bool> in_learnt_clause;
	in_learnt_clause.resize(nvars + 1, false);
	...
	if (VarDecHeuristic == VAR_DEC_HEURISTIC::ERWA_RSR || VarDecHeuristic == VAR_DEC_HEURISTIC::LRB)
	{
		for (clause_it it = new_clause.cl().begin(); it != new_clause.cl().end(); ++it)
		{
			Lit l = *it;
			...
			for (clause_it it2 = ant_clause.cl().begin(); it2 != ant_clause.cl().end(); ++it2)
			{
				Lit l2 = *it2;
				Var va2 = l2v(l2);
				if (va2 != va && !in_learnt_clause[va2])
					reasoned[va2] += 1;
			}
		}
...
void Solver::backtrack(int k) {
	...
	for (trail_t::iterator it = trail.begin() + separators[k+1]; it != trail.end(); ++it) { // erasing from k+1
		...
		if (dlevel[v]) { // we need the condition because of learnt unary clauses. In that case we enforce an assignment with dlevel = 0.
			...
			if (VarDecHeuristic != VAR_DEC_HEURISTIC::MINISAT) {
				bumpQScore(abs(v));
				m_curr_Q = max(m_curr_Q, m_Q[v]);
			}
			else m_curr_activity = max(m_curr_activity, m_activity[v]);
		}
	}
...
void Solver::restart() {	
	...
	for (unsigned int i = 1; i <= nvars; ++i) 
		if (dlevel[i] > 0) {
			...
			if (VarDecHeuristic != VAR_DEC_HEURISTIC::MINISAT) bumpQScore(i);
			dlevel[i] = 0;
		}	
	trail.clear();
	qhead = 0;
