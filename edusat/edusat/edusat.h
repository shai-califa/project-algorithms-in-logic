...
VAL_DEC_HEURISTIC ValDecHeuristic = VAL_DEC_HEURISTIC::LITSCORE;

unordered_map<string, option*> options = {
	{"v",           new intoption(&verbose, 0, 2, "Verbosity level")},
	{"timeout",     new doubleoption(&timeout, 0.0, 3600.0, "Timeout in seconds")},
...
class Solver {
	...
	// Used by LRB:
	map<double, unordered_set<Var>, greater<double>> m_Q_Score2Vars; // 'greater' forces an order from large to small of the keys
	map<double, unordered_set<Var>, greater<double>>::iterator m_Q_Score2Vars_it;
	unordered_set<Var>::iterator m_VarsSameQ_it;
	...
	vector<int> time_assigned; // # of learnt clauses at assigment
	vector<int> participated; // # of conflicts the variable participated in
	vector<int> reasoned; // # of conflicts the variable reasoned in
	vector<double> factors; // # factor to double Q in LRB var => factor
	...
	// Q and LRB	
	inline void bumpQScore(int idx);
	inline void reset_after_assigment(Var v);
	inline void reset_iterators_Q(double m_current_Q = 0.0);

public:
	Solver(): 
		nvars(0), nclauses(0), num_learned(0), num_decisions(0), num_assignments(0), 
		num_restarts(0), alpha(0.4), m_var_inc(1.0), qhead(0), 
